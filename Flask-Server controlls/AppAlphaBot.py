...
def validate(username, password):
    ...
    cur = con.cursor()
    cur.execute(f"SELECT Password FROM users_logins WHERE Utente='{username}'")
    rows = cur.fetchall()
    if len(rows) != 0:
        completion=check_password(rows[0][0], hash_string(password))
    con.close()
    return completion
...
def registra(username, password, confirm, admin):
    ...
    if admin == 0:
        cur.execute("SELECT DISTINCT users_logins.attivo FROM users_logins WHERE users_logins.Tipo = 0")
        rows = cur.fetchall()
        if len(rows) == False:
            row = True
        else: row = rows[0][0]
...
def adminReg():
    ...
    if request.method == 'POST':
        ...
        if controllaAdmin(admincode):
            esiste = registra(username, password, confirm, admin = 1)
...
def left():
    sens = gino.sensors()
    if sens == "OB_L" or sens == "OB_ALL":
        return jsonify({"Sinistra": 1})
    else:
        return jsonify({"Sinistra": 0})

@app.route('/right', methods = ['GET'])
def right():
    sens = gino.sensors()
    if sens == "OB_R" or sens == "OB_ALL":
        return jsonify({"Destra": 1})
    else:
        return jsonify({"Destra": 0})
